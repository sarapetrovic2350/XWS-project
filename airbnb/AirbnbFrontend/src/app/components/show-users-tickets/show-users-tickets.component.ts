  ...
  ngOnInit(): void {
    ...
    this.ticketService.getTicketsByUserId(this.user.id).subscribe((data: any) => {
      ...
      for (let i = 0; i < this.tickets.length; i++) {
        let dateOfPurchase = new Date(this.tickets[i].dateOfPurchase)
        this.tickets[i].dateOfPurchase = dateOfPurchase.toUTCString().replace('GMT', '')
        let dateOfDeparture = new Date(this.tickets[i].dateOfDeparture)
        this.tickets[i].dateOfDeparture = dateOfDeparture.toUTCString().replace('GMT', '')
      }
